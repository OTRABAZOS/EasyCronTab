...
function cronToHuman(schedule) {
  ...
  // Cada N horas (minuto 0, hora */N)
  if (min === '0' && hour.startsWith('*/') && dom === '*' && month === '*' && dow === '*') {
    const n = hour.slice(2);
    if (/^\d+$/.test(n)) return 'Cada ' + n + ' hora' + (n === '1' ? '' : 's');
  }
...
function cronToFormOptions(schedule) {
  ...
  if (min === '0' && hour.startsWith('*/') && dom === '*' && month === '*' && dow === '*') {
    const n = hour.slice(2);
    if (/^\d+$/.test(n)) return { frequency: 'every_n_hours', hourInterval: Math.max(1, Math.min(24, parseInt(n, 10) || 1)) };
  }
  if (dom === '*' && month === '*' && dow === '*') {
    return {
      frequency: 'daily',
